...
client.on('message', async (channel, context, message, self) => {
  ...
  if (message.startsWith(`${prefix}colors`)) {
    client.say(
      channel,
...
client.on('message', async (channel, context, message, self) => {
  ...
  if (context['custom-reward-id'] === '9c8c88c7-aec6-435f-99aa-e21fe3025f20') {
    ...
    if (!colors.includes(i)) {
      client.say(
        channel,
        `! that's not a valid color, enter "!colors" for a list of available colors!`
      );
    } else {
      client.say(channel, `/color ${message}`);
      client.say(
        channel,
        `${context.username} just changed my color to ${message}!`
      );
    }
  }
});
